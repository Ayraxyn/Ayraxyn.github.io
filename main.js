...
document.querySelector("#whitewoolCreate" +i).style.left = "850px";
    ...
    document.querySelector("#darkwoolCreate" +i).style.left = "150px";
document.querySelector("#whitewoolCreate" +i).style.left = "150px";
}
}
   // create random wools
...
document.querySelector("#" + refillableWoolID).addEventListener("click", function() {
    eeee(i, hueType);
});
}
...
function eeee(e, h) {
   woolGrab.play();
    currentWoolGrabbed = e + 1;
    showCursor(e, h); // tells showcursor which wool coord it is
}
...
ohgod(i, selector, imgSelector);
});
}
function ohgod(e, r, t) {
resetFeltingIconState();
resetFeltingPageState();
showFeltPage(r, t, e+1);
}
...
function validatorHellA(e) {
if ((windowWidth > 800) && (isImagePopUpOpen === false)) {
 imageGrowOnHover(divImage[e], images[e]);
images[e].classList.add("grow");
}
}
function validatorHellB(e) {
if ((isHoveringOverPage === true) && (isImagePopUpOpen === false)) {
    // this is to check if user clicks off when in a image page or not
    imageUnHover(divImage[e], images[e]);
images[e].classList.remove("grow");
}
}
function validatorHellC(e) {
        if (windowWidth > 800) {
    imageOpenOnClick(hiddenText[e]);
    images[e].style.display = "absolute";
images[e].classList.remove("grow");
    }
}
function validatorHellD(e) {
    imageCloseOnClick(hiddenText[e]);
images[e].classList.remove("grow");
imageUnHover(divImage[e], images[e]);
}
